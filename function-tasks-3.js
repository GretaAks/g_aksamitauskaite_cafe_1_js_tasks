...
// 12. Sukurkite funkciją, kuri grąžina balsių kiekį žodyje
console.group("12. Sukurkite funkciją, kuri grąžina balsių kiekį žodyje");
{
  function getNumberOfVowels(str) {
    ...
    for (let i = 0; i < vowels.length; i++) {
      ...
      let place;
      do {
        place = str.indexOf(vowel, lastIndex);
        lastIndex = place + 1;
        if (place !== -1) {
          vowelCount++
        }
...
// 13. Sukurkite funkciją, kuri grąžina priebalsių kiekį žodyje
console.group("13. Sukurkite funkciją, kuri grąžina priebalsių kiekį žodyje");
{
  function getNumberOfConsonants(str) {
    str = str.toLowerCase();
    const consonants = ['č', 'š', 'ž', 'q', 'w', 'r', 't', 'ę', 'p', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', 'z', 'x', 'c', 'v', 'b', 'n', 'm'];
    ...
    for (let i = 0; i < consonants.length; i++) {
      ...
      let place;
      do {
        place = str.indexOf(consonant, lastIndex);
        lastIndex = place + 1;
        if (place !== -1) {
          consonantCount++
        }
...
// 15. Sukurkite funkciją, kuri grąžina 'a' raidžių kiekį žodyje
...
// 16. Sukurkite funkciją, kuri grąžina vartotojo įvestos raidės kiekį žodyje
...
console.group("21. Sukurkite funkciją, kuri ištrintų pirmą surastą 'a' raidę žodyje ir grąžintų pakeistą žodį");
{
  ...
  function removeFirstLetterA(str) {
    const indexOfALower = str.indexOf('a'); 
    const indexOfAUpper = str.indexOf('A'); 
  ...
  function filterLetters(str, lettersToRemove) {
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      for (let j = 0; j < lettersToRemove.length; j++) {
        const letterToRemove = lettersToRemove[j];
      ...
      if (needToRemove) {
        foundLetterIndexes.push(i);
      }
    }

    let result = '';
    let from = 0;
    for (let i = 0; i < foundLetterIndexes.length; i++) {
      const letterIndexToDelete = foundLetterIndexes[i];
      result += str.slice(from, letterIndexToDelete);
      from = letterIndexToDelete + 1;
    }
    result += str.slice(from);
    return result;
  }
